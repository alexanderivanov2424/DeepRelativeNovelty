...
class Option(object):
    def __init__(self, *, name, termination_set, global_option, state_dim, action_dim, buffer_length, gestation_period, max_steps, device, option_idx, path_to_model=None):
        self.name = name
        self.device = device
    ...
    def is_init_true(self, state):
        if self.is_in_training_phase():
            return True

        features = self.extract_state_features(state)
        return self.optimistic_classifier.predict([features])[0] == 1 or self.pessimistic_classifier.predict([features])[0] == 1

    def is_in_term_set(self, state):
        state = np.reshape(state, (4,84,84))
        features = np.reshape(self.global_option.agent.model.get_features(state), (1, -1))
    ...
    def update_termination_classifier(self, nu=0.1):
        if not self.termination_set is None:
            features = [self.global_option.agent.model.get_features(obs) for obs in self.termination_set]
